...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    for region in available_region:
        keyboard.add(region)
    await message.answer("Привіт, я допоможу тобі з розкладом. Для початку вибери часовий пояс:", reply_markup=keyboard)
...
async def klass_chosen(message: types.Message, state: FSMContext):
    ...
    #await state.set_state(student.ready)
...
async def waiting_table(message: types.Message, state: FSMContext):
    user_data = await state.get_data()
    TableFinish=textget()
    if ( user_data['chosen_region']== 'київ (gmt+2)'):
        if ( user_data['chosen_klass']== '8а'):
    ...
    if ( user_data['chosen_region']== 'європа (gmt+1)'):
       if ( user_data['chosen_klass']== '8а'):
           if(message.text=="Понеділок"):
...
def register_handlers_student(dp: Dispatcher):
    ...
    dp.register_message_handler(get_table,commands="gettable", state=student.waiting_for_gettable)
    #dp.register_message_handler(get_table, commands="getTable",state=student.waiting_for_getTable)
    ...
    dp.register_message_handler(waiting_table, state=student.waiting_for_table)


#async def help_student(message: types.Message, state: FSMContext):
    #await state.finish()
    #await message.answer(
        #"Привет, я помогу тебе с расписанием, для начала нажми /student.",
...
async def set_commands(bot: Bot):
    commands = [
        #types.BotCommand(command="/student", description="Начните о себе"  ),
